...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            expanded_mask = attention_mask.to(device=logits.device, dtype=torch.bool)
            while expanded_mask.ndim < logits.ndim:
                expanded_mask = expanded_mask.unsqueeze(1)
            logits = torch.where(
                expanded_mask, logits, torch.full_like(logits, -1e30)
            )
...
class Core(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask: Boolean or binary mask of shape (B, L), where 1/True marks valid tokens.
        ...
        sequence_shape = (
            ...
            else inputs_embeds.shape[:2]  # type: ignore[union-attr]
        )
        if attention_mask is not None:
            if attention_mask.shape != sequence_shape:
                raise ValueError(
                    f"attention_mask must have shape {tuple(sequence_shape)}, "
        ...
        with _autocast_to(
            ...
            for block in self.conv_tower_blocks:
                ...
                if current_mask is not None:
                    residual_masks.append(current_mask)
                y = block.conv(x)
                y = block.res_conv(y)
                if current_mask is not None:
                    y = y * current_mask.unsqueeze(1).to(y.dtype)
                if output_hidden_states:
                    hidden_states.append(y.permute(0, 2, 1))
                x = block.avg_pool(y)
                if current_mask is not None:
                    current_mask = F.max_pool1d(
                        current_mask.unsqueeze(1).to(x.dtype),
                        kernel_size=2,
                        stride=2,
                    ).squeeze(1).bool()
                    x = x * current_mask.unsqueeze(1).to(x.dtype)
        ...
        with _autocast_to(
            ...
            for i, block in enumerate(self.deconv_tower_blocks):
                ...
                y = block(x)
                target_mask = (
                    residual_masks[-(i + 1)] if attention_mask is not None else None
                )
                if target_mask is not None:
                    y = y * target_mask.unsqueeze(1).to(y.dtype)
                if self.config.use_skip_connection:
                    y = y + r
                if target_mask is not None:
                    y = y * target_mask.unsqueeze(1).to(y.dtype)
