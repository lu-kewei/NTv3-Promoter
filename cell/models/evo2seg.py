...
class SegmentNT(nn.Module):
    def __init__(self, evo2_name, embedding_layer,embed_dim,num_layers,num_features):
        super().__init__()
        self.evo_model = Evo2(evo2_name)
    ...
    def forward(self,tokens,attention_mask):
        with torch.no_grad():
            _, embeddings = self.evo_model(tokens,return_embeddings=True, layer_names=[self.embedding_layer])
            sequence_output = embeddings[self.embedding_layer]
        ...
        logits = torch.reshape(logits, (x.shape[0], x.shape[1], self.num_features, 2))
